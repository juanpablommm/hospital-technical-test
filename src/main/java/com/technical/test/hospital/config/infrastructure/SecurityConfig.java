package com.technical.test.hospital.config.infrastructure;

import com.challenge.ecommerce.tps.filter.AuthenticationFilter;
import com.challenge.ecommerce.tps.jwt.JwtManagement;
import com.technical.test.hospital.authentication.application.create.AuthCreateAccessTokenJwt;
...
public class SecurityConfig {
	...
	@Bean
	public AuthCreateAccessTokenJwt authCreateAccessTokenJwt(JwtManagement jwtManagement) {
	...
	public AuthenticationFilter authenticationFilter(JwtManagement jwtManagement) {
		List<String> excludeUrlPatterns = List.of("/**/auth/**", "/**/refresh", "/**/refresh", "/**/swagger-ui/**",
				"/**/v3/api-docs/**");
	...
	public SecurityFilterChain filterChain(HttpSecurity http, AuthenticationProvider authenticationProvider,
		...
		http.csrf(AbstractHttpConfigurer::disable).cors(cors -> cors.configurationSource(corsConfigurationSource()))
				.authorizeHttpRequests(
						authRequest -> authRequest.requestMatchers("/auth/**", "/swagger-ui/**", "/v3/api-docs/**").permitAll().anyRequest().authenticated())
				.sessionManagement(
